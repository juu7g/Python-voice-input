...
class VoiceRecognizer():
    ...
    def recognize_voice(self, audio, i) -> str:
        ...
        try:
            model_path = r"my_model"
            text = self.r.recognize_sphinx(audio
                        , language=(os.path.join(model_path, "en-us-adapt")
                                    , os.path.join(model_path, "1347.lm")
                                    , os.path.join(model_path, "1347.dic")))
            self.my_frame.insert_msg(f"\n{i}===>{text}")
        except sr.UnknownValueError:
            logger.warning("Sphinx could not understand audio")
            self.my_frame.insert_msg("❓")
        except sr.RequestError as e:
            logger.warning("Could not request results from Sphinx service; {0}".format(e))
            self.my_frame.insert_msg("❓")
        else:
            if text == "OWARI" or text == "STOP":
...
class MyFrame(tk.Frame):
    ...
    def voice_input(self):
        ...
        with open("音声入力.txt", "a", encoding="utf_8_sig") as f:
            ...
            for future in self.vr.futures[:-1]: # 最後の「おわり」を除く
                s = future.result()
